...
void MemoryAllocator::init() {
    if(!inited){
        head = (freeNode*)((char*)HEAP_START_ADDR + ((char*)HEAP_END_ADDR - (char*)HEAP_START_ADDR)/8);
        head->size = ((char*)HEAP_END_ADDR - (char*)HEAP_START_ADDR - ((char*)HEAP_END_ADDR - (char*)HEAP_START_ADDR)/8 - sizeof(freeNode));
        head->next = nullptr;
        tail = head;
        inited = true;
    }
