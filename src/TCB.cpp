...
void TCB::dispatch()
{
    ...
    else if(old->finished){ //bilo je bez ovog else
        if(old->stack)SlabAllocator::freeBuff(old->stack);
        SlabAllocator::freeTCB(old);
    }
...
int TCB::createThread(TCB **handle, TCB::Body start_routine, void* arg, uint64 *stack) {
    ...
    TCB* newTCB = (TCB*)SlabAllocator::allocTCB();
    if(newTCB == nullptr) {
        //if(stack) MemoryAllocator::free(stack);
        *handle = nullptr;
        if(stack) SlabAllocator::freeBuff(stack);
        return -1;
    }
...
int TCB::sleep(time_t t) {
    ...
    sleepNode* sn = (sleepNode*)SlabAllocator::allocSleepNode();
    if(sn == nullptr) return -1;
...
int TCB::createCppThread(TCB** handle, Body start_routine, void* arg, uint64* stack) {
    ...
    TCB* t = (TCB*)SlabAllocator::allocTCB();
    if(t == nullptr) {
        SlabAllocator::freeBuff(stack);
        *handle = nullptr;
        return -1;
    }
...
int TCB::startCppThread(TCB *t) {
    ...
    if(!t->cppThread || t->started) return -1;
        ...
        void* stack = SlabAllocator::allocBuff(DEFAULT_STACK_SIZE);
        if(stack == nullptr) {
            return -1;
        }
        t->stack = (uint64*)stack;
        t->context.sp = (uint64)&t->stack[DEFAULT_STACK_SIZE];
        t->started = true;
        Scheduler::put(t);
        return 0;
    }
