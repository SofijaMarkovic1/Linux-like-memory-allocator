...
int Sem::open(Sem **handle, unsigned init) {
    ...
    Sem* sem = (Sem*)SlabAllocator::allocSem();
    if(sem == nullptr) {
        *handle = nullptr;
        return -1;
    }
...
void Sem::block(Sem* handle) {
    ...
    //node* n = (node*)SlabAllocator::alloc(sizeof(node));
    node* n = (node*)SlabAllocator::allocSemNode();
    if(n == nullptr) return;
    n->thread = old;
    n->next = nullptr;
    if(handle->tail) {
